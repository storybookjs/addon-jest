...
describe('defineJestParameter', () => {
  it('infers from story file name if jest parameter is not provided', () => {
    expect(defineJestParameter({ fileName: './stories/addon-jest.stories.js' })).toEqual(['addon-jest']);
  });
  ...
  it('returns as is if jest parameter is an array', () => {
    expect(defineJestParameter({ jest: ['addon-jest', 'something-else'] })).toEqual(['addon-jest', 'something-else']);
  });
